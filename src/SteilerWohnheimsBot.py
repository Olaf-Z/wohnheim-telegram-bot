...
import json
from src.utils import (ChoreType, DueDay, Chore, ChoreStatus, ChoreInformation, 
                  load_chore_data, save_chore_data, load_room_assignments, 
...
async def rotate_chores(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if incomplete_chores:
            ...
            print(f"Logged {len(incomplete_chores)} penalties for week {get_week_number()-1}")
...
def main():
    ...
    # Add job queue for reminders and rotation
    ...
    # Run chore rotation every Monday at 00:01
    job_queue.run_daily(
        rotate_chores,
        time=time(hour=3, minute=0),  # at 03:00 am, no one should be awake at this time
        days=(0,)  # Monday only
    )
